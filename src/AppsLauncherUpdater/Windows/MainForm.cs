...
public partial class MainForm : Form
{
    ...
    private void MainForm_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            foreach (var key in Ini.GetKeys("SHA256", HashInfo))
            {
                ...
                if (Ini.Read("SHA256", key, HashInfo).EqualsEx(file.EncryptFile()))
                    continue;
    ...
    private void CheckDownload_Tick(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(lastStamp))
                lastStamp = LastStamp;
            if (!Ini.Read("MD5", lastStamp, HashInfo).EqualsEx(CachePaths.UpdatePath.EncryptFile(ChecksumAlgorithm.Md5)))
