...
{
    private static volatile object _updateChecker;
    private readonly ProgressCircle _progressCircle;

    private static object UpdateChecker
    {
        get
        {
            ...
            var obj = new object();
            Interlocked.CompareExchange<object>(ref _updateChecker, obj, null);
            return _updateChecker;
        }
    }
    ...
    public AboutForm()
    {
        ...
        logoPanel.BackColor = _Settings.Window.Colors.Base.EnsureLightLight();
        ...
        aboutInfoLabel.ActiveLinkColor = _Settings.Window.Colors.Base;
    ...
    private void AddFileInfoLabels()
    {
        ...
        var strArray = new[]
        {
            new[]
            {
                CorePaths.AppsLauncher,
                CorePaths.AppsDownloader,
                CorePaths.AppsSuiteUpdater
            },
            new[]
            {
                Path.Combine(CorePaths.HomeDir, "Binaries\\SilDev.CSharpLib.dll"),
            },
            new[]
            {
                Path.Combine(CorePaths.HomeDir, "Binaries\\Newtonsoft.Json.dll"),
                Path.Combine(CorePaths.HomeDir, Environment.Is64BitProcess ? "Binaries\\Helper\\7z\\x64\\7zG.exe" : "Binaries\\Helper\\7z\\7zG.exe"),
                Path.Combine(CorePaths.HomeDir, Environment.Is64BitProcess ? "Binaries\\Helper\\7z\\x64\\7z.dll" : "Binaries\\Helper\\7z\\7z.dll")
            }
        };
        ...
        foreach (var fvi in verInfoList)
        {
            ...
            var path = new Label
            {
                ...
                Text = Path.Combine(SystemIntegration.IsEnabled ? $"%{_Settings.EnvironmentVariable}%" : ".",
    ...
    private void UpdateChecker_DoWork(object sender, DoWorkEventArgs e)
    {
        lock (UpdateChecker)
        {
            using (var updater = ProcessEx.Start(CorePaths.AppsSuiteUpdater, false, false))
            {
                if (updater?.HasExited == false)
                    updater.WaitForExit();

                ExitCode = updater?.ExitCode;
            }
            using var downloader = ProcessEx.Start(CorePaths.AppsDownloader, ActionGuid.UpdateInstance, false, false);
            if (downloader?.HasExited == false)
                downloader.WaitForExit();
            ExitCode = downloader?.ExitCode;
        }
    }
