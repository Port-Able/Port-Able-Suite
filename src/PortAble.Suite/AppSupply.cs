...
public static class AppSupply
{
    ...
    public static List<string> FindAppInstaller()
    {
        ...
        if (Directory.Exists(CachePaths.TransferDir))
            appInstaller.AddRange(searchPattern.SelectMany(x => DirectoryEx.EnumerateFiles(CachePaths.TransferDir, x)));
        return appInstaller;
    }
    ...
    public static List<AppData> FindOutdatedApps()
    {
        ...
        foreach (var appData in FindInstalledApps())
        {
            ...
            if (appData.VersionData.Count > 0)
            {
                           ...
                           .Where(tuple =>
                           {
                               ...
                               switch (tuple.data.Item2.Length)
                               {
                                   case Crypto.Md5.HashLength:
                                       fileHash = tuple.path.EncryptFile(ChecksumAlgorithm.Md5);
                                       break;
        ...
        if (Log.DebugMode > 0 && outdatedApps.Count > 0)
