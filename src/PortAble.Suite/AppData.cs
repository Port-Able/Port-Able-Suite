...
/// </summary>
[Serializable]
public sealed class AppData : IAppData, ISerializable, IEquatable<AppData>
{
    [NonSerialized]
    ...
    /// <inheritdoc cref="IAppData.InstallDir"/>
    [JsonIgnore]
    public string InstallDir
    {
        get
        {
            ...
            if (Supplier != default)
            {
                appDir = CorePaths.AppDirs[CorePaths.AppDirs.Count - 1];
                return _installDir = Path.Combine(appDir, Key.TrimEnd('#'));
            }
            if (DownloadCollection.Count < 1)
                return default;
            var downloadUrl = DownloadCollection.First().Value.FirstOrDefault()?.Item1;
            if (!string.IsNullOrWhiteSpace(downloadUrl) && NetEx.GetShortHost(downloadUrl)?.EqualsEx(AppSupplierHosts.Pa) == true)
    ...
    /// <summary>
    ///     The user agent used for internet connections.
    /// </summary>
    ...
    ///     The user agent used to download the app.
    /// </param>
    /// <param name="advanced">
    ///     The release date of the portable package.
    /// </param>
    ...
    ///     key, name, description, category, or downloadCollection is null.
    /// </exception>
    public AppData(string key, string name, string description, string category, string website,
                   ...
                   Version installerVersion, string userAgent, bool advanced,
                   CustomAppSupplier customAppSupplier = default)
    {
        ...
        if (downloadCollection == default(DownloadCollection) || downloadCollection.Values.Count < 1)
    ...
    public void GetObjectData(SerializationInfo info, StreamingContext context)
    {
        ...
        if (Languages?.Count is null or < 1 ||
            DownloadCollection?.Count is null or < 1 ||
            DownloadCollection.Values.FirstOrDefault()?.Count is null or < 1 ||
    ...
    /// <summary>
    ///     Determines whether the specified <see cref="AppData"/> instance is equal to
    ///     the current <see cref="AppData"/> instance.
    /// </summary>
    /// <param name="other">
    ///     The <see cref="AppData"/> instance to compare with the current
    ///     <see cref="AppData"/> instance.
    /// </param>
    /// <returns>
    ///     <see langword="true"/> if the specified <see cref="AppData"/> instance is
    ///     equal to the current <see cref="AppData"/> instance; otherwise
    ///     <see langword="false"/>.
    /// </returns>
    public bool Equals(AppData other) =>
        Key.EqualsEx(other?.Key) && Supplier?.Address == other?.Supplier?.Address;

    /// <inheritdoc cref="Equals(AppData)"/>
    public override bool Equals(object other)
    {
        if (other is AppData instance)
    ...
    public override int GetHashCode() =>
        Supplier == default
            ? (typeof(AppData).FullName, Key.ToUpperInvariant()).GetHashCode()
            : (typeof(AppData).FullName, Key.ToUpperInvariant(), Supplier).GetHashCode();
    ...
    public void ToString(StringBuilder sb)
    {
        ...
        const int width = 2;
        foreach (var pi in GetType().GetProperties().Where(pi => pi.CanRead))
        {
            ...
            switch (obj)
            {
                ...
                case DownloadCollection item:
                    ...
                    for (var i = 0; i < keys.Count; i++)
                    {
                        ...
                        if (item.TryGetValue(key, out var value) && value.Count > 0)
                        {
                            sb.AppendLine();
    ...
    ///     different values.
    /// </summary>
    /// <inheritdoc cref="operator ==(AppData, AppData)"/>
    public static bool operator !=(AppData left, AppData right) =>
        !Equals(left, right);
}