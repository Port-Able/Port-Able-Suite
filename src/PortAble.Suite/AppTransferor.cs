...
/// </summary>
public sealed class AppTransferor : IEquatable<AppTransferor>
{
            ...
            foreach (var (serverFile, checkHash) in pair.Value)
            {
                ...
                if (DestPath == default)
                {
                    ...
                    if (string.IsNullOrEmpty(fileName))
                        continue;
                    DestPath = PathEx.Combine(CachePaths.TransferDir, fileName);
                }
    ...
    public bool StartInstall()
    {
        ...
        foreach (var (_, checkHash, _, started) in SrcData)
        {
            ...
            if (curHash is null or nonHash)
                ...
                {
                    Crypto.Md5.HashLength => DestPath.EncryptFile(ChecksumAlgorithm.Md5),
                    Crypto.Sha1.HashLength => DestPath.EncryptFile(ChecksumAlgorithm.Sha1),
                    Crypto.Sha256.HashLength => DestPath.EncryptFile(),
    ...
    /// <summary>
    ///     Determines whether the specified <see cref="AppTransferor"/> instance is
    ///     equal to the current <see cref="AppTransferor"/> instance.
    /// </summary>
    /// <param name="other">
    ///     The <see cref="AppTransferor"/> instance to compare with the current
    ///     <see cref="AppTransferor"/> instance.
    /// </param>
    /// <returns>
    ///     <see langword="true"/> if the specified <see cref="AppTransferor"/>
    ///     instance is equal to the current <see cref="AppTransferor"/> instance;
    ///     otherwise <see langword="false"/>.
    /// </returns>
    public bool Equals(AppTransferor other) =>
        GetHashCode() == other?.GetHashCode();

    /// <inheritdoc cref="Equals(AppTransferor)"/>
    public override bool Equals(object other)
    {
        if (other is AppTransferor instance)
    ...
    public override int GetHashCode() =>
        (typeof(AppTransferor).FullName, AppData).GetHashCode();
    ...
    internal static (Version, string) FindArchivePath(string jsonArchivePath)
    {
        ...
        {
            Log.Write(ex);
            return (Version.Parse("1.0.0.0"), string.Empty);
        }
    }
    ...
    /// <summary>
    ///     Determines whether two specified <see cref="AppTransferor"/> instances have
    ///     same values.
    /// </summary>
    /// <param name="left">
    ///     The first <see cref="AppTransferor"/> instance to compare.
    /// </param>
    /// <param name="right">
    ///     The second <see cref="AppTransferor"/> instance to compare.
    /// </param>
    public static bool operator ==(AppTransferor left, AppTransferor right) =>
        Equals(left, right);
    ...
    ///     different values.
    /// </summary>
    /// <inheritdoc cref="operator ==(AppTransferor, AppTransferor)"/>
    public static bool operator !=(AppTransferor left, AppTransferor right) =>
        !Equals(left, right);
}