...
{
    private static volatile object _downloadHandler;
    private static volatile object _downloadStarter;
    private static volatile object _searchHandler;
    ...
    public AppsDownloaderJson Settings { get; } = new();

    private static object DownloadHandler
    {
        get
        {
            ...
            var obj = new object();
            Interlocked.CompareExchange<object>(ref _downloadHandler, obj, null);
            return _downloadHandler;
        }
    }

    private static object DownloadStarter
    {
        get
        {
            ...
            var obj = new object();
            Interlocked.CompareExchange<object>(ref _downloadStarter, obj, null);
            return _downloadStarter;
        }
    }

    private static object SearchHandler
    {
        get
        {
            ...
            var obj = new object();
            Interlocked.CompareExchange<object>(ref _searchHandler, obj, null);
            return _searchHandler;
        }
    }
    ...
    public MainForm(NotifyBox notifyBox = default)
    {
        ...
        appsList.RemoveDottedSelectionBorders();
        appsList.EnableExplorerSelectionStyle();
        appsList.SetDoubleBuffer();

        CacheData.SetComponentImageColor(appMenuItem1);
        CacheData.SetComponentImageColor(appMenuItem2);
        appMenuItem3.Image = appMenuItem2.Image;
        CacheData.GetImage(nameof(Resources.Uncheck), Resources.Uncheck, SystemColors.Highlight.InvertRgb());
    ...
    private void MainForm_Load(object sender, EventArgs e)
    {
        ...
        if (!ActionGuid.IsUpdateInstance && AppSupplierMirrors.Pa.Count < 1)
        {
    ...
    {
        if (_transferManager.Count > 0 && MessageBoxEx.Show(this, LangStrings.AreYouSureMsg, MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
        {
            e.Cancel = true;
            return;
        }
        ...
        foreach (var file in DirectoryEx.EnumerateFiles(Settings.TransferDir))
            CmdExec.WaitThenDelete(file);
    }
    ...
    private void AppsList_ItemCheck(object sender, ItemCheckEventArgs e)
    {
        ...
        if (appData == null || appData.Requirements.Count < 1)
            return;
    ...
    private void AppMenuItem_Click(object sender, EventArgs e)
    {
        ...
        switch (owner?.Name)
        {
            ...
            {
                var appData = CacheData.AppInfo.FirstOrDefault(x => x.Key.EqualsEx(selectedItem.Name));
                var (result, language) = DefineInstallerLanguage(appData, selectedItem);
    ...
    private void SettingBtn_MouseEnterLeave(object sender, EventArgs e)
    {
        if (sender is not Panel owner)
            return;
        owner.BackgroundImage = owner.BackgroundImage.SwitchAlpha(owner);
    }
    ...
    private void StartBtn_Click(object sender, EventArgs e)
    {
        ...
        foreach (var item in appsList.CheckedItems.Cast<ListViewItem>())
        {
            ...
            _transferManager.Add(item, new AppTransferor(appData, this));
        ...
        var homeDrive = CorePaths.HomeDir.First();
        var transferDrive = Settings.TransferDir.First();
        if (Settings.TransferDir.StartsWithEx(homeDrive))
            spaceData = new[]
            {
                (homeDrive, unchecked(totalDownloadSize + totalInstallSize), DirectoryEx.GetFreeSpace(CorePaths.HomeDir))
            };
        else
            spaceData = new[]
            {
                (transferDrive, totalDownloadSize, DirectoryEx.GetFreeSpace(Settings.TransferDir)),
                (homeDrive, totalInstallSize, DirectoryEx.GetFreeSpace(CorePaths.HomeDir))
            };
        foreach (var (driveLetter, neededSize, availableSize) in spaceData)
        {
            if (neededSize < availableSize)
                continue;
            var warning = LangStrings.NotEnoughSpaceMsg.FormatInvariant(driveLetter, (neededSize - availableSize).FormatSize());
    ...
    private void DownloadHandler_Tick(object sender, EventArgs e)
    {
        ...
        lock (DownloadHandler)
        {
            ...
            */
            MessageBoxEx.Show(this, LangStrings.OperationCompletedMsg, MessageBoxButtons.OK, MessageBoxIcon.Information);
            ApplicationExit();
        }
    }
    ...
    private void AppsListUpdate(List<AppData> appInfo = default)
    {
        ...
        if (CacheData.CustomAppSuppliers?.Count is > 0)
        {
        ...
        foreach (var appData in appInfo ?? CacheData.AppInfo)
        {
            ...
            if (appData.Supplier != null)
            {
                ...
                while (true)
                {
                    foreach (var group in from lvg in appsList.Groups.Cast<ListViewGroup>()
                                          let str = lvg.Header
            ...
            else
            {
                ...
                foreach (var group in from lvg in appsList.Groups.Cast<ListViewGroup>()
                                      let str = lvg.Header
                                      where (!appData.Advanced && str.EqualsEx(appData.Category)) || str.EqualsEx(advanced)
                                      select lvg)
                {
                    appsList.Items.Add(item).Group = group;
                    break;
    ...
    private (DialogResult, string) DefineInstallerLanguage(AppData appData, ListViewItem item, bool needed = false)
    {
        ...
        if (!(appData?.DownloadCollection.Count > 1) || appData.Settings.ArchiveLangConfirmed)
            return (DialogResult.OK, selected);
