...
public partial class SourceManagerForm : Form
{
    ...
    private void SaveBtn_Click(object sender, EventArgs e)
    {
        ...
        _ = FileEx.DeleteAll(DirectoryEx.EnumerateFiles(dir));
        ...
        if (DirectoryEx.GetFiles(dir) is { Length: > 0 } files)
            foreach (var supplier in files.Select(f => CacheData.LoadDat<CustomAppSupplier>(f)).Where(x => x != default))
